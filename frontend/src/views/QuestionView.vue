...
<template>
  <section class="page">
    ...
    <div v-else class="question-list"><article v-for="item in shown" :key="item.id" class="paper-card question-card"><div class="question-meta"><span class="mono">{{ item.questionCode }}</span><span class="type-pill">{{ typeLabel(item.type) }}</span><b>{{ item.totalScore }} 分</b></div><p class="formula-text"><MathText :text="item.content" /></p><div class="question-foot"><span>主知识点：{{ points.find(point => point.id === item.primaryKnowledgePointId)?.name || item.primaryKnowledgePointId }}</span><span v-if="item.type === 'SOLUTION'">{{ item.rubricItems.length }} 个评分步骤</span><span v-else>{{ item.acceptedAnswers.length }} 个可接受答案</span></div></article></div>
