...
- [x] 内存数据库演示模式；
- [x] 浏览器主路径验收；
- [ ] 真实第三方模型联调（仍缺真实密钥与服务授权。第二阶段补的是适配器契约测试，
      用本地模拟服务验证请求与脱敏边界，不能代替真实服务联调）；
- [x] 持久化 MySQL 集成验证 —— 第二阶段已用一次性 `mysql:8.4` 容器验证 V1–V6 迁移与
      核心工作流（`mvn verify -Pmysql-it`，7 个用例）。注意：这**不等于**在持久化 MySQL 上
      部署过，真实业务数据的迁移仍未执行；
- [x] 分层练习生成与 Word 导出 —— 第二阶段里程碑三完成（三级分层、教师确认、可打印 Word）；
- [ ] 毕业论文中的离线评价实验 —— 评测脚本、匿名样本格式和实验记录模板已完成（第二阶段
      任务七，`evaluation/`），但还没有用真实匿名样本跑过实验，论文数字尚未产生。

剩余两项都卡在外部输入上：真实模型密钥与服务授权、真实匿名评分样本。
本文件是第一阶段的状态快照，第二阶段的结果见
`docs/2026-09-14-项目阶段总结.md` 第八节与同目录的第二阶段实施计划。
